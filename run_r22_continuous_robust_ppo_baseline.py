...
RUN_FAMILY_ROOT = ROOT / "experiments" / "r22_continuous_robust_ppo"
POINTER = RUN_FAMILY_ROOT / "latest_r22_continuous_robust_ppo.txt"
SWEEP_SCRIPT = ROOT / "lqr_theta_sweep_eval.py"

SELECTED_SYSTEM_LABEL = "r22_1p5"
CONTINUOUS_THETA_RANGE = [-0.25, 0.25]
EVAL_THETA_POINTS = 41
DEFAULT_SEEDS = [1, 2, 3]
...
def _smoke_enabled() -> bool:
    return os.environ.get("R22_CONTINUOUS_ROBUST_PPO_SMOKE", "0").lower() in {"1", "true", "yes"}
...
def _selected_seeds() -> list[int]:
    raw = os.environ.get("R22_CONTINUOUS_ROBUST_PPO_SEEDS", "")
    if raw.strip():
...
def _set_continuous_theta_wrappers(cfg: dict[str, Any]) -> None:
    """Randomize theta uniformly over the closed interval each episode."""
    for wrapper in cfg.get("wrappers", []):
        if wrapper.get("enabled", True) is False:
            continue
        if wrapper.get("name") not in {"DomainRandomizationWrapper", "ChangingCartPoleDynamics"}:
            continue
        ...
        params["theta_mult_range"] = list(CONTINUOUS_THETA_RANGE)
        # The shared adapter validates every range against the global sampling
        # mode, including disabled parameter families.
        params["mu_range"] = [0.25, 0.75]
        params["a_range"] = [0.75, 1.05]
        params["b_range"] = [0.4, 1.8]
        params["process_noise_scale_mult_range"] = [1.0, 1.0]
...
def _robust_phase_cfg(
    *,
    ...
    _set_continuous_theta_wrappers(cfg)
...
def _verify_written_config(expected: dict[str, Any], *, phase: str) -> None:
    ...
    _assert_equal(wrapper.get("theta_mult_range"), CONTINUOUS_THETA_RANGE, f"{phase}.theta_mult_range")
    _assert_equal(wrapper.get("categorical"), False, f"{phase}.categorical")
    for key in ["mu_range", "a_range", "b_range", "process_noise_scale_mult_range"]:
        values = wrapper.get(key)
        if not isinstance(values, list) or len(values) != 2:
            raise AssertionError(f"{phase}: {key} must contain two continuous-range bounds, got {values!r}")
...
def _train_seed(
    *,
    ...
    exp_name = f"s_{stamp}_{system['label']}_continuous_robust_ppo_seed{seed}{smoke_suffix}"
...
def _run_sweep(*, target: Path, specs: list[dict[str, Any]], output_subdir: str) -> Path:
    ...
    # Do not set THETA_SWEEP_THETA_VALUES: the evaluator derives the interval
    # from the saved config and constructs a dense deterministic grid.
    env.pop("THETA_SWEEP_THETA_VALUES", None)
    env["THETA_SWEEP_N_THETA_POINTS"] = str(EVAL_THETA_POINTS)
...
def _evaluate(run_root: Path, seed_results: list[dict[str, Any]]) -> None:
    ...
    for result in seed_results:
        ...
        final_dir = _run_sweep(
            ...
            output_subdir=f"r22_continuous_robust_ppo_seed{seed}/final",
        )
        ...
        checkpoint_dir = _run_sweep(
            ...
            output_subdir=f"r22_continuous_robust_ppo_seed{seed}/all_checkpoints",
        )
...
def main() -> None:
    ...
    if _smoke_enabled():
        run_root = Path("/private/tmp") / f"r22_continuous_robust_ppo_smoke_{stamp}"
    ...
    payload: dict[str, Any] = {
        ...
        "theta_distribution": {
            ...
            "range": list(CONTINUOUS_THETA_RANGE),
        },
        "setup": {
            "non_adaptive": True,
            ...
            "n_steps": TRAIN_N_STEPS,
            "batch_size": TRAIN_BATCH_SIZE,
            "evaluation_theta_points": EVAL_THETA_POINTS,
