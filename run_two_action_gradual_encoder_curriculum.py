...
RUN_SUFFIX = "two_action_gradual_encoder_curriculum"
ARCH = [128, 128]
...
def _smoke_enabled() -> bool:
    return os.environ.get("TWO_ACTION_GRADUAL_ENCODER_SMOKE", "0").lower() in {"1", "true", "yes"}
...
def _set_gradual_encoder_curriculum(
    cfg: dict,
    *,
...
def _write_pipeline_file(exp_dir: Path, payload: dict) -> None:
    ...
    with open(exp_dir / "two_action_gradual_encoder_curriculum.yaml", "w") as f:
...
def main() -> None:
    ...
    payload = {
        ...
        "method": "Episode-level curriculum from privileged (theta, 0) to encoder-predicted (mean, std) context",
        "encoder_training": "NLL only; PPO context gradients detached",
