...
PRIVILEGED_PHASES = [
    ("A_start_visible", "start_visible", True, 1_000_000, 0.04, 3e-4),
    ("B_easy", "easy", False, 1_200_000, 0.03, 3e-4),
    ("C_medium", "medium", False, 1_500_000, 0.02, 2e-4),
    ("D_hard_final", "final", False, 2_000_000, 0.01, 1e-4),
]

GRADUAL_PHASES = [
    ("P01", "easy", 0.25, 0.04, 3e-4, 900_000),
    ("P02", "medium", 0.50, 0.03, 3e-4, 1_000_000),
    ("P03", "final", 0.75, 0.02, 2e-4, 1_200_000),
    ("P04", "final", 1.00, 0.01, 1e-4, 1_500_000),
    ("P05", "final", 1.00, 0.002, 1e-4, 1_000_000),
]
...
def _train_privileged(base_cfg: dict, run_dir: Path) -> Path:
    ...
    for idx, (phase_name, env_phase, start_visible, raw_steps, ent, lr) in enumerate(_privileged_phases()):
        ...
        run_training_stage(f"v2 privileged/{phase_name}", cfg)
...
def _train_gradual_nll(
    ...
    *,
    ...
    for idx, (phase_name, env_phase, enc_prob, ent, lr, raw_steps) in enumerate(_gradual_phases()):
        ...
        run_training_stage(f"v2 {name}/{phase_name}", cfg)
        _snapshot(exp_dir, phase_name)
        evaluate_model(
            exp_dir,
...
def main() -> None:
    ...
    stamp = timestamp()
    run_dir = OUT_ROOT / f"friction_stopping_v2_{stamp}"
    run_dir.mkdir(parents=True, exist_ok=True)
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    (OUT_ROOT / "latest_friction_stopping_v2.txt").write_text(str(run_dir.resolve()) + "\n")
    ...
    try:
        print(f"Writing friction-stopping v2 outputs to {run_dir}", flush=True)
        ...
        _train_gradual_nll(
            ...
            run_dir,
            privileged_dir,
            name="gradual_nll_mean_std_v2",
            condition_on_uncertainty=True,
        )
        print(f"Friction-stopping v2 run complete: {run_dir}", flush=True)
    finally:
