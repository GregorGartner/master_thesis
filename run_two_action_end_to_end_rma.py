...
RUN_SUFFIX = "two_action_e2e_latent_rma_scratch"
ARCH = [128, 128]
LATENT_DIM = 4
...
def _smoke_enabled() -> bool:
    return os.environ.get("TWO_ACTION_E2E_RMA_SMOKE", "0").lower() in {"1", "true", "yes"}
...
def _set_end_to_end_rma(cfg: dict, *, ent: float, lr: float) -> None:
...
def _write_pipeline_file(exp_dir: Path, payload: dict) -> None:
    ...
    with open(exp_dir / "two_action_end_to_end_rma.yaml", "w") as f:
...
def main() -> None:
    ...
    try:
        for phase_idx, (phase, ent, lr, steps) in enumerate(phases):
            ...
            _set_end_to_end_rma(cfg, ent=ent, lr=lr)
            ...
            _run_stage(f"e2e_rma/{phase}", cfg, exp_dir, payload)
