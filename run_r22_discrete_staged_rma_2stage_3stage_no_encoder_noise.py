...
RUN_FAMILY_ROOT = ROOT / "experiments" / "r22_discrete_staged_rma_no_encoder_noise"
POINTER = RUN_FAMILY_ROOT / "latest_r22_discrete_staged_rma_no_encoder_noise.txt"
...
def _smoke_enabled() -> bool:
    return (
        os.environ.get("R22_STAGED_RMA_NO_ENCODER_NOISE_SMOKE", "0").lower()
        in {"1", "true", "yes"}
    )
...
def _train_seed(
    *,
    ...
    names = {
        "privileged": f"s_{stamp}_{system['label']}_discrete_staged_no_noise_shared_priv_seed{seed}{smoke_suffix}",
        "two_stage": f"s_{stamp}_{system['label']}_discrete_two_stage_rma_no_noise_seed{seed}{smoke_suffix}",
        "three_stage": f"s_{stamp}_{system['label']}_discrete_three_stage_rma_no_noise_seed{seed}{smoke_suffix}",
    }
...
def main() -> None:
    ...
    if _smoke_enabled():
        run_root = Path("/private/tmp") / f"r22_discrete_staged_rma_no_encoder_noise_smoke_{stamp}"
        exp_root = run_root / "experiments"
    else:
        RUN_FAMILY_ROOT.mkdir(parents=True, exist_ok=True)
        run_root = RUN_FAMILY_ROOT / f"staged_rma_2stage_3stage_no_encoder_noise_{stamp}_{system['label']}"
    ...
    payload: dict[str, Any] = {
        ...
        "setup": {
            ...
            "encoder_only_naive_action_noise_std": 0.0,
            "encoder_only_naive_action_noise_dist": "gaussian",
        },
        "phase_budgets": {
            "privileged": [
