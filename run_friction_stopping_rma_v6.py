...
CALIBRATION_NOISE_STD = [0.0, 0.30]
...
def _adaptive_phases() -> list[tuple[str, str, str, float, float, float, int, bool]]:
    if smoke_enabled():
        return SMOKE_ADAPTIVE_PHASES
...
def _train_privileged(base_cfg: dict, run_dir: Path) -> Path:
    ...
    for idx, (phase_name, env_phase, start_visible, raw_steps, ent, lr) in enumerate(_privileged_phases()):
        ...
        run_training_stage(f"v6 privileged/{phase_name}", cfg)
...
def _set_calibration_policy(
    ...
    *,
    ...
    params["naive_action_noise_std"] = CALIBRATION_NOISE_STD
    params["naive_action_noise_dist"] = CALIBRATION_NOISE_DIST
...
def _train_adaptive_nll(
    ...
    *,
    ...
    for idx, (phase_name, phase_kind, env_phase, enc_prob, ent, lr, raw_steps, freeze_encoder) in enumerate(_adaptive_phases()):
        ...
        run_training_stage(f"v6 {name}/{phase_name}", cfg)
        _snapshot(exp_dir, phase_name)
        evaluate_model(
            exp_dir,
...
def main() -> None:
    ...
    stamp = timestamp()
    run_dir = OUT_ROOT / f"friction_stopping_v6_{stamp}"
    run_dir.mkdir(parents=True, exist_ok=True)
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    (OUT_ROOT / "latest_friction_stopping_v6.txt").write_text(str(run_dir.resolve()) + "\n")
    ...
    save_json(
        run_dir / "run_manifest.json",
        {
            ...
            "adaptive_phases": [
                ...
                for phase_name, phase_kind, env_phase, enc_prob, ent, lr, raw_steps, freeze_encoder in _adaptive_phases()
            ],
            "calibration_noise_std": CALIBRATION_NOISE_STD,
            "calibration_noise_dist": CALIBRATION_NOISE_DIST,
            "final_visible_distance": [0.9, 1.2],
            "focused_methods": ["gradual_nll_mean_only_v6", "gradual_nll_mean_std_v6"],
            "privileged_gate": {
                "max_crash_rate": PRIVILEGED_GATE_MAX_CRASH_RATE,
                "min_success_rate": PRIVILEGED_GATE_MIN_SUCCESS_RATE,
                "max_pre_bound_frac": PRIVILEGED_GATE_MAX_PRE_BOUND_FRAC,
                "ignore_env_var": "FRICTION_STOPPING_V6_IGNORE_GATE",
            },
        },
    )

    try:
        print(f"Writing friction-stopping v6 outputs to {run_dir}", flush=True)
        ...
        print(f"Friction-stopping v6 privileged gate: {gate_payload}", flush=True)

        ignore_gate = os.environ.get("FRICTION_STOPPING_V6_IGNORE_GATE", "0").lower() in {"1", "true", "yes"}
        if not gate_payload.get("passed", False) and not smoke_enabled() and not ignore_gate:
            print(
                "Stopping v6 before adaptive training because privileged PPO did not pass the reliability/bound gate. "
                "Set FRICTION_STOPPING_V6_IGNORE_GATE=1 to override.",
                flush=True,
            )
            return
        ...
        mean_std_dir = _train_adaptive_nll(
            ...
            run_dir,
            privileged_dir,
            name="gradual_nll_mean_std_v6",
            condition_on_uncertainty=True,
        )

        evaluations = run_dir / "evaluations"
        final_phase = "FT01" if smoke_enabled() else "FT02"
        mean_only_final_eval = evaluations / f"gradual_nll_mean_only_v6_{final_phase}"
        mean_std_final_eval = evaluations / f"gradual_nll_mean_std_v6_{final_phase}"
        write_final_comparison_diagnostics(
            run_dir,
            model_evaluations={
        ...
        print(f"Friction-stopping v6 run complete: {run_dir}", flush=True)
